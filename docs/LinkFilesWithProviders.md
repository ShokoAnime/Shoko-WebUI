...
Editable fields: `Version`, `Source`, `IsChaptered`, `IsCreditless`, `Comment`, and `Release Group`. The release group is a searchable `SelectReleaseGroup` combobox that filters known groups by name/shortname and only shows options while typing; typing an unknown name offers a `Create "…"` option that sets the group's `Name`, `ShortName`, and `ID` to the typed value with `Source: User`. Episode selection supports multiple rows. The first row is the primary selector and includes an "Auto-match (from filename)" option (`AUTO_MATCH_EPISODE_ID = -1`) and, directly below it, a "Range Fill" option (`RANGE_FILL_EPISODE_ID = -2`), whose label reads "Range Fill (from S3)" once a range is configured (episode prefix via `getEpisodePrefix` + starting number) and "Range Fill (not set)" otherwise. Selecting "Range Fill" (from the dropdown or via the `r` hotkey) automatically opens the `RangeFillModal` (episode type + starting number); a pencil edit button, shown only while "Range Fill" is selected, reopens it. On save, consecutive episodes of the chosen type are assigned to the selected files in sorted order. Additional concrete episode rows are added with a plus button (enabled only once the first row has a concrete episode selected) and removed with the minus button on each added row; each row's options come from `buildConcreteEpisodeOptions(rowIdx)`, which disables episodes already selected in another row, and selecting auto-match or range fill in the first row clears all added rows. When selected files have differing episode links, the first row shows a disabled "Multiple episodes selected" entry and those per-file links are preserved unless an episode (or auto-match/range-fill) is explicitly chosen; in bulk mode, when every selected link shares the same series and an identical cross-reference set (compared via a joined `xrefSetKey`), the rows are pre-filled from the first link's existing cross-references. The release group field shows "Multiple groups selected" for mixed groups. Save is gated by a `saveDisabled` flag (`!show`, no touched fields, "Range Fill" selected without a configured range, or any added row missing a concrete episode) that disables the Save button (which itself only renders while `hasSeriesSelection`) and is also checked at the top of `handleSave` so `Enter` triggers save safely.

State is managed by the `useReleaseInfoForm` hook (form state, touched-field tracking, mixed-selection flags). Clearing the release group or comment and leaving the field removes the stored value on save — `setIfTouched` uses a `writeUndefined` option so an empty value is omitted from the payload and cleared server-side. On save, `handleSaveReleaseInfo` in the page merges the patch into each selected link's `release`, resolves the episode cross-references (auto-match resolves via `detectShow` → matching episode; concrete rows are used as-is), and appends the `+User` marker to `ProviderName` (unless `isUserEdited`). The per-link mutation (apply the `releaseInfo` patch, write `CrossReferences`, append `+User`, transition to `ready`) lives in a local `applyRelease(link, episodeIds)` helper shared by the auto-match/single-episode and range-fill branches; for multi-episode selections it writes one cross-reference per concrete episode ID, splitting the 0–100% range across them with `getEpisodePercentageRange` (floor-based: 2 episodes → 0–50/51–100, 3 episodes → 0–33/34–66/67–100). A link's state is set to `ready` only when at least one concrete episode is written; when no `crossReference` was provided (a release-only save), or auto-match fails to find an episode, the link keeps its previous state (no `ready` transition) while the release patch is still merged and `+User` appended. Failed auto-matches — where a `crossReference` was present but resolution failed — are reported in an error toast with the file count. For range fill, the selected links are iterated in sorted (`RelativePath`) order and each gets the next consecutive episode of the chosen type starting from the configured number; if the episode queue runs out before all selected files are processed, a warning toast reports how many files could not be filled.
