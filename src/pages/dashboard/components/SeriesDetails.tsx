...
function SeriesDetails(props: { series: SeriesType }): JSX.Element {
  ...
  return (
    <div key={`series-${series.IDs.ID}`} className="mr-4 last:mr-0 shrink-0 w-56 justify-center flex flex-col">
      <BackgroundImagePlaceholderDiv imageSrc={`/api/v3/Image/${seriesImageSource}/Poster/${seriesImageID}`} className="relative h-80 rounded drop-shadow-[0_4px_4px_rgba(0,0,0,0.25)] border border-panel-border mb-3" />
