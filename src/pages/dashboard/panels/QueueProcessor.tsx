          ...
          {item?.status === 'Pausing' || item?.status === 'Paused'
            ? (
              <Button className="mx-2" onClick={() => handleOperation('Start', key)} tooltip="Resume">
                <Icon className="text-panel-text-primary" path={mdiPlayCircleOutline} size={1} />
              </Button>
            )
            : (
              <Button className="mx-2" onClick={() => handleOperation('Stop', key)} tooltip="Pause">
                <Icon className="text-panel-text-primary" path={mdiPauseCircleOutline} size={1} />
              </Button>
            )}
          <Button className="mx-2" onClick={() => handleOperation('Clear', key)} tooltip="Clear">
            <Icon className="text-panel-text-primary" path={mdiCloseCircleOutline} size={1} />
          </Button>
        </div>
      </div>
    ...
    return (
      <>
        {paused
          ? (
            <div
              className="mx-2 cursor-pointer text-panel-text-primary"
        ...
        <Button className="mx-2" onClick={handleOpenQueueDialog} tooltip="Open Queue Modal">
          <Icon className="text-panel-text-primary" path={mdiTextBoxOutline} size={1} />
        </Button>
      </>
    );
  };
