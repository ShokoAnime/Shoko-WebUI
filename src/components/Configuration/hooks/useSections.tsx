...
import { pathToString, resolveReference } from '@/core/schema';
...
function useSections(
  ...
  return useMemo(() => {
    ...
    const { actions, sectionAppendFloatingAtEnd, sectionName: defaultSectionName = 'Default', sectionType, showSaveAction } =
    ...
    if (performAction) {
      for (const [actionId, action] of Object.entries(actions)) {
        ...
        const actionPath = pathToString([...path, actionId]);
        ...
        const actionButton = (
          <Button
            key={key}
            loading={loadingActions.includes(actionPath)}
            disabled={disabled}
            buttonType={action.theme as ButtonType}
    ...
    for (
      ...
      const uiDefinition = { ...value['x-uiDefinition']!, ...resolvedSchema['x-uiDefinition']! };
    ...
    // Add the default save action to all tabs if this is a tab section container.
    if (sectionType === 'tab') {
      if (defaultSave && showSaveAction) {
        const actionButton = (
          <Button
        ...
        for (const section of sections) {
          section.buttons.bottom.push(actionButton);
        }
      }
