...
const PopOutModal = ({ show, ...props }: PopOutModalProps) => {
  ...
  const setConfig = (rootConfig: unknown) => {
  ...
  const performCustomAction = (path: (string | number)[], actionName: string) => {
  ...
  const performReactiveAction = (newConfig: unknown, path: (string | number)[]) => {
  ...
  useLayoutEffect(() => {
    ...
    if (
      props.index !== undefined && props.index === state.index && state.rootConfig && state.parentConfig && state.config
    ) {
      if (!state.shown) {
        ...
        setState({
          shown: true,
          path: [...props.path, parentConfig.length - 1],
          index: props.index,
          rootConfig,
          parentConfig,
          config,
