...
const ListTabPills = (props: ListTabPillsProps): React.JSX.Element => {
  ...
  return (
    <div className="flex gap-x-3 overflow-x-scroll">
      ...
      {!showAddButton && tabs.length === 0 && (
        <div className="group relative min-w-0 shrink-0 grow place-items-center overflow-hidden rounded-lg bg-panel-background p-4 text-sm font-medium whitespace-nowrap text-panel-toggle-text-alt focus:z-10">
      ...
      {showAddButton && (
        <button
          ...
          onClick={addItem}
          className={cx(
            'group relative flex min-w-0 shrink-0 place-items-center overflow-hidden rounded-lg bg-panel-background p-4 text-sm font-medium whitespace-nowrap text-panel-toggle-text-alt hover:bg-panel-toggle-background-hover focus:z-10',
...
const TabPill = (props: TabPillProps): React.JSX.Element => {
  const { canRemove, index: tabIdx, removeItem, showRemoveButton, tab, tabCount } = props;

  const isDisabled = tab.active || tabCount <= 1;
  ...
  return (
    <button
      ...
      className={cx(
        'group relative flex min-w-0 shrink-0 grow overflow-hidden p-4 text-center text-sm font-medium whitespace-nowrap focus:z-10',
        tab.active
