...
const SeriesList = (
  ...
  const seriesColumns = useMemo<UtilityHeaderType<ReleaseManagementSeriesType>[]>(() => [
    {
      id: 'series',
      name: t('releaseManagement.seriesList.seriesColumn'),
    ...
    {
      id: 'entry-count',
      name: t('releaseManagement.seriesList.entryCount'),
      className: 'w-32',
      item: (series) => {
        ...
        return (
          <>
            ...
            {t('releaseManagement.common.entry', { count })}
          </>
        );
      },
    },
  ], [t]);
  const episodeNameColumn = useMemo<UtilityHeaderType<EpisodeType>>(() => ({
    id: 'episode',
    name: t('releaseManagement.seriesList.episodeName'),
  ...
  const multiplesEpisodeFileCountColumn = useMemo<UtilityHeaderType<EpisodeType>>(() => ({
    id: 'file-count',
    name: t('releaseManagement.seriesList.fileCount'),
    className: 'w-28',
    item: (episode) => {
      ...
      return (
        <>
          <span className="text-panel-text-important">{count}</span>
          {t('releaseManagement.common.file', { count })}
        </>
      );
    },
  }), [t]);
  const duplicatesEpisodeFileCountColumn = useMemo<UtilityHeaderType<EpisodeType>>(() => ({
    id: 'duplicate-count',
    name: t('releaseManagement.seriesList.duplicateCount'),
    className: 'w-40',
    item: (episode) => {
      let count = 0;
      ...
      return (
        <>
          <span className="text-panel-text-important">{count}</span>
          {t('releaseManagement.common.duplicate', { count })}
        </>
      );
    },
  }), [t]);
  ...
  const episodeColumns = useMemo(() => {
    ...
    return [
      ...
      {
        id: 'selected-count',
        name: selectedRows.length > 0 ? t('releaseManagement.seriesList.selected', { count: selectedRows.length }) : '',
  ...
  return (
    <>
      <div className="flex w-1/2 overflow-y-auto rounded-md border border-panel-border bg-panel-background p-6">
        ...
        {!seriesQuery.isPending && seriesCount === 0 && (
          <div className="flex grow items-center justify-center text-lg font-semibold">
            {t('releaseManagement.seriesList.noSeriesPrefix')}
            {type === ReleaseManagementItemType.MultipleReleases && t('releaseManagement.seriesList.noSeriesMultiples')}
            {type === ReleaseManagementItemType.DuplicateFiles && t('releaseManagement.seriesList.noSeriesDuplicates')}
            {type === ReleaseManagementItemType.MissingEpisodes && t('releaseManagement.seriesList.noSeriesMissing')}
          </div>
        )}
      ...
      <div className="flex w-1/2 overflow-y-auto rounded-md border border-panel-border bg-panel-background p-6">
        {selectedSeries === 0 && (
          <div className="m-auto text-lg font-semibold">{t('releaseManagement.seriesList.selectSeries')}</div>
        )}
