...
const ConfigModal = (props: Props) => {
  ...
  const handlePipeNameChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (lockedControls) return;
    setPipeName(event.target.value);
  };

  const handlePipeClick = (event: React.MouseEvent<HTMLElement>) => {
    ...
    if (selectedPipe1 && selectedPipe1.ID === selectedPipe?.ID) {
          ...
          {pipes?.map(pipe => (
            <div
              ...
              className={cx(
                ...
                !lockedPipe && defaultPipeId === pipe.ID && (selectedPipe?.ID !== pipe.ID)
