...
function Checkbox({ id, label, isChecked, className, onChange, labelRight, justify, intermediate }: Props) {
  ...
  return (
    <label htmlFor={id} className={cx([justify && 'justify-between', `${className}`, 'cursor-pointer flex items-center transition ease-in-out', focused && 'ring-2 ring-default-primary ring-inset'])}>
      <input
        id={id}
        type="checkbox"
        checked={isChecked}
      ...
      {!intermediate && isChecked && (
        <TransitionDiv className="flex text-default-primary" enterFrom="opacity-50" appear={false}>
