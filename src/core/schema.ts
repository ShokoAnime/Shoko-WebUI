...
export function createDefaultItemForSchema(
  ...
  const types = Array.isArray(resolvedSchema.type)
    ? resolvedSchema.type.filter(tpe => tpe !== 'null')
    : [resolvedSchema.type];
  if (types[0] !== 'object') {
    if (types[0] === 'array') {
      if (resolvedSchema.minItems !== undefined && resolvedSchema.minItems > 0) {
        return new Array(resolvedSchema.minItems).fill(0)
          .map((_, index) =>
            createDefaultItemForSchema(rootSchema, resolveListReference(rootSchema, resolvedSchema), resolvedSchema, [
              ...path,
              index,
            ]));
      }
